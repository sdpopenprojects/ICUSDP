...
from algorithms.AutoEncoder import VariationalAutoencoder
...
class InterpretableMultiViewClusteringVAE:
    """
    Interpretable Multi-View Clustering Algorithm with Variational Autoencoders

    Based on the paper: "Interpretable multi-view clustering" by Jiang et al.
    This algorithm combines variational autoencoders with decision trees for interpretable clustering.

    Key features:
    - Learns embedded features for each view using VAEs
    ...
    def __init__(self, n_clusters=2, hidden_dims=[128, 64], latent_dim=32,
                 e1_epochs=200, e2_epochs=200, lambda_ce=0.1, lambda_kld=0.01, random_state=42):
        """
        Initialize the interpretable multi-view clustering model with VAEs
        ...
        # Model components
        self.vaes = []  # List of VAEs for each view
    ...
    def _pretrain_vaes(self, X_list):
        """
        Pre-train VAEs for each view independently
        ...
        for v, X in enumerate(X_list):
            # print(f"Pre-training VAE for view { v +1}...")

            # Convert to PyTorch tensor
            X_tensor = torch.FloatTensor(X)

            # Create VAE for current view
            input_dim = X.shape[1]
    ...
    def _extract_embedded_features(self, X_list):
        ...
        for v, (X, vae) in enumerate(zip(X_list, self.vaes)):
            X_tensor = torch.FloatTensor(X)
            vae.eval()
    ...
    def _construct_initial_decision_tree(self, X_original, pseudo_labels):
        ...
        # self.randomforest = RandomForestClassifier(random_state=self.random_state)
        # self.randomforest.fit(X_original, pseudo_labels)
        # self.feature_importances_ = self.randomforest.feature_importances_
    ...
    def fit_predict(self, X_list, max_iters=10):
        ...
        #
        # print("=" * 60)
        # print("Interpretable Multi-View Clustering with VAEs")
        ...
        self._pretrain_vaes(X_normalized_list)
        ...
        # initial_pseudo_labels = SC(Z_concat)
        ...
        for iteration in range(max_iters):
            ...
            # tree_labels = self.randomforest.predict(X_original_concat)
        ...
        # final_labels = self.randomforest.predict(X_original_concat)
    ...
    def get_interpretability_report(self, feature_names, threshold=0.01):
        """
        ...
        feature_importances_ = self.decision_tree.feature_importances_
        ...
        # tree_rules = export_text(self.decision_tree, feature_names=custom_names)

        tree_rules = export_text(self.decision_tree, feature_names=feature_names)

        # View importance analysis
        # n_features_per_view = [vae[0].shape[1] for vae in [self.vaes]]  # Approximation
        #
        # view_importances = []
        # start_idx = 0
        # for n_features in n_features_per_view:
        #     end_idx = start_idx + n_features
        #     if end_idx <= len(self.feature_importances_):
        #         view_importance = np.sum(self.feature_importances_[start_idx:end_idx])
        #         view_importances.append(view_importance)
        #         start_idx = end_idx
        #
        # # If we have more views than feature groups, distribute remaining importance
        # remaining_views = len(self.vaes) - len(view_importances)
        # if remaining_views > 0:
        #     avg_importance = np.mean(view_importances) if view_importances else 1.0 / len(self.vaes)
        #     view_importances.extend([avg_importance] * remaining_views)
    ...
    def parse_tree_rules(self, tree_text):
        """Parse export_text output to understand tree structure"""
        ...
        for line in lines:
            ...
            tree_structure.append({
                'depth': depth,
                'text': line_clean,
                'is_leaf': 'class' in line_clean.lower()
            })

        return tree_structure

    def print_tree_hierarchical(self, tree_structure):
        """Print tree in hierarchical format"""
        for node in tree_structure:
            indent = "  " * node['depth']
            prefix = "├── " if node['depth'] > 0 else "• "
            print(f"{indent}{prefix}{node['text']}")

    def generate_latent_samples(self, X_list, n_samples=10):
        """
        ...
        # Use first view for sampling (or could combine views)
        X_sample = X_list[0][:n_samples]
