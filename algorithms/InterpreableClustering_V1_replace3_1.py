...
class InterpretableClustering:
    ...
    def _init_interpreter(self, input_dim):
        ...
        # ==================== 【修改点 2：更改分支，支持传入 RF, GBM, XGBOOST】 ====================
        elif self.interpreter_type == 'RF':
            self.interpreter = RFC(random_state=self.random_state)
        elif self.interpreter_type == 'GBM':
            self.interpreter = GBC(random_state=self.random_state)
        elif self.interpreter_type == 'XGBOOST':
            self.interpreter = XGBC(random_state=self.random_state, eval_metric='logloss')
    ...
    def visualize_decision_rules(self, feature_names=None):
        ...
        # ==================== 【修改点 3：让新模型统一输出 Feature Importance 解释文本】 ====================
        elif self.interpreter_type in ['RF', 'GBM', 'XGBOOST']:
            summary = f"{self.interpreter_type} Global Feature Importance:\n"
            importances = self.tree.feature_importances_
            for name, imp in zip(feature_names, importances):
                summary += f"  {name}: {imp:.4f}\n"
            return summary
