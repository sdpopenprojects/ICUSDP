...
def perm_importance(model, X, y):
    ...
    for i in range(len(X)):
        ...
        for j in range(x.shape[1]):
            ...
            predict_y1 = [labelCluster(x, clus_label) for x in X1]
            predict_y1 = np.mean(predict_y1, axis=0)

            predict_y2 = labelCluster_v2(clus_label)

            temp_acc1 = sum(predict_y1 == y)/n
            acc1.append(temp_acc1)

            temp_acc2 = sum(predict_y2 == y)/n
            acc2.append(temp_acc2)

            # predict_y = 0.5 * predict_y1 + 0.5 * predict_y2
