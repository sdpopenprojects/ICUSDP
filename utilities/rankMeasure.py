...
def rank_measure(predict_score, effort, test_label):
    ...
    (cErecall, cEprecision, cEfmeasure, cMCC, cPMI, cIFA, cPCI, ceIFA,
     mRecall, mPrecision, mfmeasure, mMCC, mPMI, mIFA, mPCI, meIFA) = computeMeasure(data_mdl, length)

    # 💡 为了计算修正后的 ROI = TP / PCI 和 ROI = TP / PII
    # 我们需要获取测试集中的总缺陷数（即 test_label 中所有 1 的总和）作为基础
    total_bugs = np.sum(test_label)

    # --- 1. 基于 20% 代码行 (c系列) 的 ROI ---
    # 衡量：在 20% Effort 预算下的投资回报率
    # 根据定义：cErecall = tp_c / total_bugs => tp_c = cErecall * total_bugs
    tp_c = cErecall * total_bugs

    # 🎯 修正分子为实际抓到的缺陷绝对数量 TP
    c_ROI_PII = tp_c / cPMI if cPMI != 0 else 0
    ...
    # --- 2. 基于 20% 模块数 (m系列) 的 ROI 修正 ---
    # 根据定义：mRecall = tp_m / total_bugs => tp_m = mRecall * total_bugs
    tp_m = mRecall * total_bugs

    # 🎯 修正分子为实际抓到的缺陷绝对数量 TP
    m_ROI_PII = tp_m / mPMI if mPMI != 0 else 0
...
def computeMeasure(data, length):
    ...
    return (cErecall, cEprecision, cEfmeasure, cMCC, cPMI, cIFA, cPCI, ceIFA,
            mRecall, mPrecision, mfmeasure, mMCC, mPMI, mIFA, mPCI, meIFA)
