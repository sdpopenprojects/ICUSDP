...
# ==================== 1. 定义指定的绝对路径 ====================
input_dir = r"F:\ICUSDP\INTC\ICUSDP\W_C_test\visual\Radar"
output_dir = r"F:\ICUSDP\INTC\ICUSDP\W_C_test\visual\Radar"

excel_path = os.path.join(input_dir, "all.xlsx")
output_pdf_path = os.path.join(output_dir, "Radar_Comparison.pdf")
...
# --- 核心修改点 2：锁定指定的 4 个核心度量指标，将 F1 替换为 cIFA ---
metrics = ['AUC', 'cIFA', 'MCC', 'cEfmeasure']
df = df_raw[metrics].copy()

# --- 核心修改点 3：cIFA 物理语义是越小越好，取负号转化为越大越好（便于雷达图统一向外扩张） ---
df['cIFA'] = -df['cIFA']
...
# ==================== 3. 划分三大阵营并计算中位数 ====================
unsupervised_list = ['ONE', 'CLA', 'CLAMI', 'KMedoids', 'MD', 'MU', 'SC', 'TCL', 'TCLP']
supervised_list = ['DT', 'GBM', 'linearSVM', 'LR', 'RF', 'XGBoost']

unsupervised_profile = df_norm.loc[df_norm.index.isin(unsupervised_list)].median(axis=0).values.tolist()
supervised_profile = df_norm.loc[df_norm.index.isin(supervised_list)].median(axis=0).values.tolist()
...
ax.plot(angles, unsupervised_profile, color='#1976D2', linewidth=2.2, linestyle='--',
        marker='s', markersize=5, label='Unsupervised Baselines (Median)')
ax.fill(angles, unsupervised_profile, color='#1976D2', alpha=0.05)

# --------- 5.2 绘制监督阵营中位数线 (学术绿，点划线) ---------
ax.plot(angles, supervised_profile, color='#388E3C', linewidth=2.2, linestyle='-.',
        marker='^', markersize=5, label='Supervised Baselines (Median)')
ax.fill(angles, supervised_profile, color='#388E3C', alpha=0.04)

# --------- 5.3 绘制 ICUSDP 核心主模型 (高亮学术红，破圈粗实线) ---------
ax.plot(angles, icusdp_profile, color='#D32F2F', linewidth=4.0, linestyle='-',
        marker='o', markersize=8, zorder=10, label='ICUSDP (Ours)')
...
# 应用大字号标签
ax.set_xticklabels(display_labels, fontsize=RADAR_LABEL_SIZE, fontweight='bold', color='#262626')
...
# 使用 bbox_inches='tight' 确保放大字号后的边缘文字与图例不被截断掉
plt.savefig(output_pdf_path, dpi=300, bbox_inches='tight')
print(f"🎉 【大字号替换为 cIFA 雷达图绘制成功】已无损导出至: {output_pdf_path}")
plt.show()