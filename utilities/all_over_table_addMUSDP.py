...
# ==================== 路径与配置区域 ====================
INPUT_DIR = r"E:\ICUSDP\INTC\ICUSDP\W_C_test\statistic\all_before"
OUTPUT_DIR = r"E:\ICUSDP\INTC\ICUSDP\W_C_test\statistic\all_over1"

# 1. 将 KMedoids 替换为 MUSDP（保持 16 个模型）
MODEL_COLUMNS = [
    'ICUSDP', 'ONE', 'CLA', 'CLAMI', 'MUSDP', 'MD', 'MU', 'SC',
    'TCL', 'TCLP', 'DT', 'GBM', 'linearSVM', 'LR', 'RF', 'XGBoost'
]

# 2. MUSDP 专属的 13 个指标名称映射（请确保顺序与 MUSDP 原始 CSV 的 13 列完全对齐）
MUSDP_13_METRICS = [
...
def extract_model_name(file_name):
    ...
    if raw_extracted == "musdp": return "MUSDP"
    ...
    for metric_name in sorted(all_data.keys()):
        ...
        for proj in PROJECT_ORDER:
            ...
            for model in MODEL_COLUMNS:
                # 若某个模型（如 MUSDP）没有当前指标，自动填为 np.nan，不影响其他模型整合
        ...
        # 显示是否包含 MUSDP 的提示信息
        has_musdp = "包含 MUSDP" if df_metric_table['MUSDP'].notna().any() else "仅整合其他15个方法 (MUSDP无此指标)"
        print(f"表格生成成功 -> [{file_count}]: {safe_metric_name}.xlsx ({has_musdp})")

    print("\n==============================================================")
    print(f"🎉 处理完成！KMedoids 已替换为 MUSDP。")
    print(f"📂 所有指标 Excel 已更新并输出至: {OUTPUT_DIR}")
    print("==============================================================")
