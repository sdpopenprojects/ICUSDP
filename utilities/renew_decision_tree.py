...
def main():
    ...
    # 创建输出目录
    output_dir = 'renew_decision_tree'
    ...
    for i, file_path in enumerate(pkl_files):
        ...
        try:
            ...
            # 7.1 检查是否有训练数据和特征重要性，以便重新训练排序后的决策树
            if 'X_train' in data and 'y_train' in data and hasattr(tree_model, 'feature_importances_'):
                print("  检测到训练数据和特征重要性，将按特征重要性排序重新训练决策树...")
                ...
                # 选择最重要的前N个特征（可以根据需要调整）
                top_n = min(20, len(sorted_indices))  # 取前20个最重要的特征
                top_indices = sorted_indices[:top_n]

                print(f"  使用最重要的{top_n}个特征重新训练决策树:")
                for i, idx in enumerate(top_indices):
                    feat_name = feature_names[idx] if idx < len(feature_names) else f"feature_{idx}"
                    print(f"    {i + 1}. {feat_name}: {importances[idx]:.4f}")

                # 使用排序后的特征数据
                X_train_sorted = X_train[:, top_indices]
                ...
                # 获取原树的参数，或使用简化参数
                if hasattr(tree_model, 'max_depth') and tree_model.max_depth is not None:
                    max_depth = min(5, tree_model.max_depth)  # 限制深度以便可视化
                else:
                    max_depth = 5

                # 创建新的决策树
                sorted_tree = DecisionTreeClassifier(
                    max_depth=max_depth,
                    random_state=42  # 固定随机种子保证可重复
                )

                sorted_tree.fit(X_train_sorted, y_train)
                print(f"  重新训练的决策树深度: {sorted_tree.get_depth()}")
                print(f"  重新训练的决策树节点数: {sorted_tree.tree_.node_count}")

                # 8. 可视化排序后的决策树
                plot_decision_tree_sklearn(sorted_tree, sorted_feature_names, output_filename)
            else:
                # 如果没有训练数据，使用原树
                print("  未检测到训练数据或特征重要性，使用原决策树")
                # 8. 可视化原决策树
