...
SK_SUPERVISED_DIR = r'E:\ICUSDP-main\ICUSDP-main\new2\SDP'
SK_UNSUPERVISED_DIR = r'E:\ICUSDP-main\ICUSDP-main\new2\USDP'
...
def get_sk_groups(metric_name, is_supervised=True):
  ...
  if not os.path.exists(sk_file):
    print(
        f'⚠️ 未找到 SK 结果文件: {sk_file}，将退化为普通无分组显示。'
    )
    return None
...
def plot_single_category_sk(
  ...
  sns.boxplot(
      ...
      flierprops=dict(
          marker='o',  # 圆圈
          markersize=4.5,  # 圆圈大小
          markerfacecolor='none',  # 空心圆（若要实心圆可改为 'gray' 或 'black'）
          markeredgecolor='#444444',  # 边框深灰色，比纯黑更优雅
          alpha=0.7,  # 半透明感
      ),
  )
...
def main():
  ...
  for idx, file_path in enumerate(excel_files, 1):
    ...
    try:
      ...
      print(
          f'[{idx}/{total_files}] 成功生成基于 SK-ESD 分组的 2 张独立排序图（PNG &'
          f' PDF）：[{metric_name}]'
      )
    except Exception as e:
